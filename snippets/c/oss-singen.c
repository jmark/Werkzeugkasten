...
write_sinewave (void) {
  ...
  static unsigned int phase2 = 1000;    /* Phase of the sine wave */
  int i;
  ...
  static int sinebuf[48] = {
    ...
    for (i = 0; i < outsz; i++) {
        unsigned int p   = (phase * sample_rate) / 48000;
        unsigned int p2  = (phase2 * sample_rate) / 48000;
        phase           = (phase + 1) % 4800;
        phase2          = (phase + 1) % 4800;
        buf[i]          = sinebuf[p2 % 48]+sinebuf[p % 48];
    }
